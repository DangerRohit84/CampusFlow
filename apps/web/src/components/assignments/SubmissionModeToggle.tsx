...
export default function SubmissionModeToggle({ value, onChange }: Props) {
  return (
    <div className="space-y-2">
      <label className="block text-sm font-semibold text-surface-700 dark:text-night-300">How will students submit? <span className="text-danger-500">*</span></label>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
        {opts.map(o => {
          ...
          const Icon = o.icon
          return (
            <button key={o.id} onClick={()=> onChange(o.id)} className={`p-4 rounded-xl border-2 text-left transition-all ${active?'border-primary-500 bg-primary-50 dark:bg-primary-900/20 shadow-sm':'border-surface-200 dark:border-night-700 bg-white dark:bg-night-800 hover:border-surface-300 dark:hover:border-night-600'}`}>
              <Icon size={18} className={active?'text-primary-600 dark:text-primary-400':'text-surface-500'} />
              <div className={`text-sm font-bold mt-2 ${active?'text-primary-900 dark:text-primary-100':'text-surface-900 dark:text-night-50 dark:text-night-50'}`}>{o.title}</div>
              <div className={`text-xs font-medium mt-1 ${active?'text-primary-700 dark:text-primary-300':'text-surface-600 dark:text-night-300'}`}>{o.plain}</div>
