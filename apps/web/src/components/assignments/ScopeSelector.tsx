...
export default function ScopeSelector({ value, departmentId, roomId, onChange }: Props) {
  ...
  return (
    <div className="space-y-3">
      <div>
        <label className="block text-sm font-semibold text-surface-700 dark:text-[#A6B3BE]">Who can see this assignment? <span className="text-danger-500">*</span></label>
        ...
        {options.map(o => {
          ...
          return (
            <button
              ...
              className={`p-3 rounded-xl border-2 text-left transition-all ${active ? 'border-primary-500 bg-primary-50 dark:bg-primary-900/20 shadow-sm' : 'border-surface-200 dark:border-[#202C35] bg-white dark:bg-[#111920] hover:border-surface-300 dark:hover:border-[#2A3A44]'}`}
            >
              <Icon size={16} className={active?'text-primary-600 dark:text-primary-400':'text-surface-500'} />
              <div className={`text-sm font-semibold mt-1.5 leading-tight ${active?'text-primary-900 dark:text-primary-100':'text-surface-900 dark:text-night-50 dark:text-[#F4F7F8]'}`}>{o.title}</div>
      ...
      {value==='DEPARTMENT' && (
        <div className="space-y-1.5">
          <label className="block text-xs font-semibold text-surface-700 dark:text-[#A6B3BE]">Pick a department <span className="text-danger-500">*</span></label>
          {loadingDept ? <div className="text-xs text-surface-500 dark:text-night-400 py-2">Loading departments…</div> :
            <select value={departmentId||''} onChange={e=> onChange({ scope: value, departmentId: e.target.value||null, roomId: null })} className="w-full px-4 py-3 bg-surface-50 dark:bg-[#0D151C] border border-surface-200 dark:border-[#202C35] rounded-xl text-sm focus:outline-none focus:border-primary-400 focus:ring-2 focus:ring-primary-500/20">
      ...
      {value==='ROOM' && (
        <div className="space-y-1.5">
          <label className="block text-xs font-semibold text-surface-700 dark:text-[#A6B3BE]">Pick a classroom / room <span className="text-danger-500">*</span></label>
          {loadingRoom ? <div className="text-xs text-surface-500 dark:text-night-400 py-2">Loading classrooms…</div> :
            <select value={roomId||''} onChange={e=> onChange({ scope: value, departmentId: null, roomId: e.target.value||null })} className="w-full px-4 py-3 bg-surface-50 dark:bg-[#0D151C] border border-surface-200 dark:border-[#202C35] rounded-xl text-sm focus:outline-none focus:border-primary-400 focus:ring-2 focus:ring-primary-500/20">
      ...
      {value==='ALL' && (
        <p className="text-xs flex gap-2 p-2.5 rounded-lg bg-surface-50 dark:bg-[#0D151C] border border-surface-200 dark:border-[#202C35] text-surface-600 dark:text-[#A6B3BE]"><Info size={14} className="shrink-0 mt-0.5"/> Every student in your college will see this on their Assignments page.</p>
      )}
    </div>
  )
}
