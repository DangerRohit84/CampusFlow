...
export default function AssignmentHubCard({ hub, onClick, onEdit, onDelete }: any) {
  ...
  const dueTone = due==='Overdue'?'text-danger-600':due==='Today'||due==='Tomorrow'?'text-amber-600':'text-surface-600 dark:text-[#A6B3BE]'
  ...
  return (
    <Card hover onClick={onClick} className="group cursor-pointer">
      <div className="flex gap-4">
        <div className={`w-1 rounded-full shrink-0 ${isOverdue?'bg-danger-500':'bg-primary-500'}`} />
        <div className="flex-1 min-w-0">
          <div className="flex items-start justify-between gap-3">
            <div className="min-w-0 flex-1">
              <div className="flex items-center gap-2 flex-wrap">
                <h3 className="font-bold text-surface-900 dark:text-night-50 dark:text-[#F4F7F8] group-hover:text-primary-700 dark:group-hover:text-primary-300 leading-tight">{hub.title}</h3>
                ...
                <span className="inline-flex items-center gap-1 text-xs text-surface-500 dark:text-[#A6B3BE] px-2 py-0.5 rounded-full bg-surface-50 dark:bg-[#0D151C] border border-surface-200 dark:border-[#1e2e3a]" title={modePlain}><ModeIcon size={12}/> {modePlain}</span>
              </div>
              <p className="text-xs text-surface-500 dark:text-[#A6B3BE] mt-1">{hub.courseId ? `Course: ${hub.courseId}` : 'General assignment'} • For: {scopePlain}</p>
              {hub.description && <p className="text-xs text-surface-500 dark:text-[#8A9BA8] mt-1 line-clamp-1">{hub.description}</p>}
            </div>
          ...
          <div className="flex items-center gap-2 mt-2.5 flex-wrap">
            ...
            {!hub.showGrades && <span className="inline-flex items-center gap-1 text-xs text-surface-400 dark:text-night-400 px-2 py-0.5 rounded-full bg-surface-50 dark:bg-[#0D151C] border border-surface-200 dark:border-[#202C35]"><EyeOff size={10}/> Grades hidden</span>}
            {!hub.showFeedback && <span className="inline-flex items-center gap-1 text-xs text-surface-400 dark:text-night-400 px-2 py-0.5 rounded-full bg-surface-50 dark:bg-[#0D151C] border border-surface-200 dark:border-[#202C35]"><EyeOff size={10}/> Feedback hidden</span>}
        ...
        {(onEdit||onDelete) && <div className="flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity self-start"><button aria-label="Edit assignment" onClick={e=>{e.stopPropagation(); onEdit?.(hub)}} className="p-2 rounded-lg hover:bg-surface-100 dark:hover:bg-[#202C35] text-surface-500 hover:text-surface-900 dark:text-night-50">✎</button><button aria-label="Delete assignment" onClick={e=>{e.stopPropagation(); onDelete?.(hub)}} className="p-2 rounded-lg hover:bg-danger-50 text-danger-500">✕</button></div>}
      </div>
    </Card>
  )
}
