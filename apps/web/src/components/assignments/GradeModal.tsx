    ...
    <Modal open={open} onClose={onClose} title={`Grade: ${submission?.student?.name || submission?.studentId}`}>
      <div className="space-y-4">
        <div className="flex flex-wrap items-center gap-2 text-xs">
          ...
          {submission?.status && <span className="px-2 py-1 rounded-full bg-surface-100 dark:bg-night-700 border dark:border-night-600 text-surface-500">{submission.status}</span>}
        </div>
        ...
        {submission?.fileUrl && (()=> {
          ...
          return <div className="space-y-1.5">{urls.map((u,i)=> <a key={i} href={u} target="_blank" rel="noreferrer" className="flex items-center gap-1.5 text-sm text-primary-600 underline"><span className="px-1.5 py-0.5 rounded bg-surface-100 dark:bg-night-700 border text-xs">Page {i+1}</span>{names[i]}</a>)} {submission.fileType==='multi' && <span className="text-xs text-surface-500">{urls.length} pages</span>}</div>
        })()}
