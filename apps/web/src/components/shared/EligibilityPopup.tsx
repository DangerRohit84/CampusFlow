      ...
      {show && (
        ...
        >
          ...
          >
            ...
            <p className="text-sm text-surface-500 mb-5">{subtitle}</p>

            {/* Mode buttons */}
            {showRoomsMode && showDepartmentMode && setEligibilityMode && (
              <div className="flex gap-3 mb-4">
                <button onClick={() => setEligibilityMode('rooms')} className={clsx('flex-1 p-3 rounded-xl border-2 transition-all', eligibilityMode === 'rooms' ? 'border-primary-500 bg-primary-50 text-primary-700' : 'border-surface-200 hover:border-surface-300 text-surface-600')}>
                ...
                <button onClick={() => setEligibilityMode('department')} className={clsx('flex-1 p-3 rounded-xl border-2 transition-all', eligibilityMode === 'department' ? 'border-primary-500 bg-primary-50 text-primary-700' : 'border-surface-200 hover:border-surface-300 text-surface-600')}>
                  <div className="text-center">
                    <span className="text-2xl block mb-1">🏫</span>
                  ...
                  rooms.map(room => (
                    <label key={room.id} className="flex items-center gap-3 p-2.5 rounded-xl border hover:bg-surface-50 cursor-pointer transition-all">
                      ...
                      <div className="flex-1">
                        ...
                        <p className="text-xs text-surface-400">{room._count?.members || room.memberCount || 0} members</p>
                      </div>
                    </label>
                  ))
                )}
                  ...
                  <div className="flex flex-wrap gap-1.5">
                    {departments.map(d => (
                      <button key={d.id} type="button" onClick={() => setTargetDepartments(targetDepartments.includes(d.id) ? targetDepartments.filter(id => id !== d.id) : [...targetDepartments, d.id])}
                        className={clsx('px-3 py-1.5 rounded-lg text-xs font-medium border transition-all', targetDepartments.includes(d.id) ? 'bg-primary-100 border-primary-300 text-primary-700' : 'bg-white border-surface-200 text-surface-600 hover:border-primary-200')}>
                        {d.name}
                      </button>
                    ))}
                    {departments.length === 0 && <p className="text-xs text-surface-400">No departments created yet.</p>}
                  </div>
                </div>
                <div>
                  ...
                  <div className="flex gap-2">
                    {[1, 2, 3, 4].map(y => (
                      <button key={y} type="button" onClick={() => setTargetYears(targetYears.includes(y) ? targetYears.filter(n => n !== y) : [...targetYears, y])}
                        className={clsx('w-12 h-9 rounded-lg text-sm font-bold border transition-all', targetYears.includes(y) ? 'bg-primary-100 border-primary-300 text-primary-700' : 'bg-white border-surface-200 text-surface-600 hover:border-primary-200')}>
                        {y}
                      </button>
                    ))}
                  </div>
