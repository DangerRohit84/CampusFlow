...
export default function TeacherAssignedPage() {
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      const [hackData, intData] = await Promise.all([
        hackathonAPI.getStaging(1, 50, undefined, signal),
        internshipAPI.getStaging(1, 50, undefined, signal),
      ])
      if (!isCurrent(seq) || signal.aborted) return
      // Filter to only items assigned to this teacher
      setHackathons((hackData as any).data ? (hackData as any).data.filter((h: any) => h.assignedTeacherId === user?.id) : (hackData as any[]).filter((h: any) => h.assignedTeacherId === user?.id))
      setInternships((intData as any).data ? (intData as any).data.filter((i: any) => i.assignedTeacherId === user?.id) : (intData as any[]).filter((i: any) => i.assignedTeacherId === user?.id))
    } catch (err: any) {
      if (isAbortError(err, signal)) return
      const msg = err?.response?.data?.error || 'Could not load assigned opportunities.'
      setLoadError(msg)
      toast.error(msg)
    } finally {
  ...
  useEntitySync(['hackathon', 'internship'], loadData as any)
  ...
  const handleApprove = async (id: string, type: 'HACKATHON' | 'INTERNSHIP') => {
    ...
    try {
      if (type === 'HACKATHON') {
        ...
        notifyEntityMutated('hackathon', { stagingId: id, action: 'approved' })
      } else {
        await internshipAPI.approveStaging(id)
        setInternships((prev) => prev.filter((i) => i.id !== id))
        notifyEntityMutated('internship', { stagingId: id, action: 'approved' })
      }
  ...
  const handleReject = async (id: string, type: 'HACKATHON' | 'INTERNSHIP') => {
    const ok = await confirmDialog({ title: 'Reject opportunity?', message: 'Reject this opportunity?', confirmLabel: 'Reject' })
    if (!ok) return
    try {
      if (type === 'HACKATHON') {
        await hackathonAPI.deleteStaging(id)
        setHackathons((prev) => prev.filter((h) => h.id !== id))
        notifyEntityMutated('hackathon', { stagingId: id, action: 'rejected' })
      } else {
        await internshipAPI.deleteStaging(id)
        setInternships((prev) => prev.filter((i) => i.id !== id))
        notifyEntityMutated('internship', { stagingId: id, action: 'rejected' })
      }
  ...
  // TOPBOTTOM F10: students have their own boards (/hackathons /internships) —
  // the teacher review queue must not render as a confusing empty list.
  if (user?.role === 'STUDENT') {
    return (
      <div className="min-h-screen bg-surface-50 dark:bg-night-800">
        <h1 className="sr-only">Assigned to Me — Review opportunities assigned to you</h1>
        <div className="max-w-3xl mx-auto px-4 py-12">
          <div className="rounded-2xl border border-surface-200 dark:border-night-600 bg-white dark:bg-night-800 p-8 text-center" role="alert">
            <h2 className="text-lg font-bold text-surface-900 dark:text-night-50">Teachers only</h2>
            <p className="text-sm text-surface-500 dark:text-night-400 mt-2">This review queue is for faculty. Browse open opportunities instead.</p>
            <div className="mt-5 flex justify-center gap-2">
              <Link to="/hackathons" className="px-4 py-2.5 rounded-xl bg-primary-600 text-white text-sm font-semibold">Browse hackathons</Link>
              <Link to="/internships" className="px-4 py-2.5 rounded-xl border border-surface-200 dark:border-night-600 text-sm font-semibold">Browse internships</Link>
            </div>
          </div>
        </div>
      </div>
  ...
  if (loadError) {
    return (
      <div className="min-h-screen bg-surface-50 dark:bg-night-800">
        ...
        <div className="max-w-3xl mx-auto px-4 py-12">
          <div className="rounded-2xl border border-surface-200 dark:border-night-600 bg-white dark:bg-night-800 p-8 text-center" role="alert">
            <h2 className="text-lg font-bold text-surface-900 dark:text-night-50">Couldn&apos;t load assigned work</h2>
            <p className="text-sm text-surface-500 dark:text-night-400 mt-2">{loadError}</p>
            <button onClick={() => loadData()} className="mt-5 px-5 py-2.5 rounded-xl bg-surface-900 text-white text-sm font-semibold">Retry</button>
          </div>
        </div>
      </div>
    )
