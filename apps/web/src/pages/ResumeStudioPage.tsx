...
import { Download, Save, Trash2, Copy, Globe, FileText, Loader2, Share2, FileCode, Layers, Upload, BarChart3, Target, FileUp, X, Lightbulb, Sparkles, History, Github, Mail, Link2, Check, RotateCcw } from 'lucide-react'
...
import { diffWords, hasDiffChanges } from '../lib/resume/wordDiff'
import { analyzeResume, applyMetricSuggestion } from '../lib/resume/metricCoach'
import { listVersions, saveVersion, restoreVersion, deleteVersion, type ResumeVersion } from '../lib/resume/versions'
import { reposToProjects, mergeProjects, type GithubRepoBrief } from '../lib/resume/githubImport'
...
function TailorChangeRow({ label, before, after, onApply }: { label: string; before: string; after: string; onApply: () => void }) {
  ...
  if (!hasDiffChanges(tokens)) return null
  return (
    <div className="rounded-xl border border-surface-200 dark:border-night-600 px-3 py-2">
      <div className="flex items-center justify-between gap-2">
        <p className="text-xs font-bold text-surface-800 dark:text-night-100">{label}</p>
        <button onClick={onApply} className="inline-flex items-center gap-1 px-3 py-1.5 rounded-full bg-violet-600 hover:bg-violet-700 text-white text-[11px] font-semibold shrink-0">
          <Check size={11} /> Apply
        </button>
      </div>
      <p className="mt-1 text-xs leading-relaxed text-surface-700 dark:text-night-200">
        {tokens.map((t, i) => (
          <span key={i} className={
            t.type === 'add' ? 'bg-emerald-100 dark:bg-emerald-500/20 text-emerald-800 dark:text-emerald-200 rounded px-0.5' :
            t.type === 'del' ? 'bg-red-100 dark:bg-red-500/20 text-red-700 dark:text-red-300 line-through rounded px-0.5' : ''
          }>{t.text}{' '}</span>
        ))}
      </p>
    </div>
...
export default function ResumeStudioPage() {
  ...
  // #10 resume depth — JD scrape-from-URL (grounded verbatim text, no AI)
  const [jdUrl, setJdUrl] = useState('')
  const [jdScraping, setJdScraping] = useState(false)
  const [jdSource, setJdSource] = useState<string | null>(null)
  // #10 — tailor word-diff preview with Apply-per-change (pending, not auto-applied)
  const [preTailor, setPreTailor] = useState<ResumeData | null>(null)
  const [pendingTailor, setPendingTailor] = useState<any | null>(null)
  // #10 — versions (localStorage snapshots; see lib/resume/versions.ts decision note)
  const [versions, setVersions] = useState<ResumeVersion[]>([])
  // #10 — cover letter (resume + JD via existing AI path, heuristic fallback)
  const [showCoverModal, setShowCoverModal] = useState(false)
  const [coverTone, setCoverTone] = useState<'professional' | 'enthusiastic' | 'concise'>('professional')
  const [coverLetter, setCoverLetter] = useState('')
  const [coverLoading, setCoverLoading] = useState(false)
  const [coverUsedAI, setCoverUsedAI] = useState(false)
  // #10 — GitHub import (public repos → projects via backend, no token)
  const [showGhModal, setShowGhModal] = useState(false)
  const [ghUsername, setGhUsername] = useState('')
  const [ghLoading, setGhLoading] = useState(false)
  const [ghRepos, setGhRepos] = useState<GithubRepoBrief[]>([])
  ...
  useEffect(() => {
    const anyModal = showAtsModal || showJDModal || showCoverModal || showGhModal
    if (anyModal) document.body.style.overflow = 'hidden'
    else document.body.style.overflow = ''
    return () => { document.body.style.overflow = '' }
  }, [showAtsModal, showJDModal, showCoverModal, showGhModal])
  ...
  useEffect(() => {
    ...
    setVersions(listVersions(userId))
    setPreTailor(null)
    setPendingTailor(null)
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [userId])

  // #10 — Metric Coach flags (derived, pure) + version count refresh helper
  const metricFlags = useMemo(() => analyzeResume(data), [data])
  const refreshVersions = useCallback(() => setVersions(listVersions(userId)), [userId])
  ...
  const handleClear = async () => {
    const ok = await confirmDialog({ title: 'Clear resume?', message: 'Clear all resume data? This cannot be undone.', confirmLabel: 'Clear all' })
    if (!ok) return
  ...
  const handleAIUpgrade = useCallback(async (mode: string, extra?: { jd?: string; targetId?: string }) => {
    ...
    try {
      ...
      if (mode === 'summary' && result.summary) {
        ...
        // #10 — do NOT auto-apply: stage as pending + show word-diff preview with
        // Apply-per-change below (grounded: only AI-returned fields become changes).
        const incoming = result as any
        setPreTailor(data)
        if (incoming.rawText) {
          setPendingTailor({ rawText: String(incoming.rawText).slice(0, 3000), suggestions: incoming.suggestions || [] })
          toast.success('AI tailor suggestions ready — review diff below, apply per change')
        } else {
          const staged: any = {}
          if (incoming.personalInfo?.summary) staged.summary = incoming.personalInfo.summary
          if (incoming.personalInfo?.headline) staged.headline = incoming.personalInfo.headline
          if (Array.isArray(incoming.skills) && incoming.skills.length) staged.skills = incoming.skills
          if (Array.isArray(incoming.experience)) {
            staged.experienceBullets = incoming.experience
              .filter((e: any) => e?.id && Array.isArray(e.bullets) && e.bullets.length)
              .map((e: any) => ({ id: e.id, bullets: e.bullets }))
          }
          if (incoming.certifications?.length) staged.certifications = incoming.certifications
          staged.suggestions = incoming.suggestions || []
          if (!staged.summary && !staged.skills && !staged.experienceBullets?.length) {
            toast('AI returned no concrete changes — try a longer JD', { icon: '💡' })
            setPreTailor(null)
            setPendingTailor(null)
          } else {
            setPendingTailor(staged)
            toast.success('Tailor preview ready — apply per change below ✨')
          }
        }
      } else {
        // generic
  ...
  // #10 — JD scrape-from-URL: paste a posting link → verbatim JD text (grounded, no AI).
  const handleScrapeJd = useCallback(async () => {
    ...
    setJdScraping(true)
    try {
      const res = await resumeAPI.scrapeJd(url)
      if (res.text) {
        setJdText(res.text)
        setJdSource(res.sourceUrl)
        toast.success(`JD fetched — ${res.text.length} chars${res.truncated ? ' (truncated to 6k)' : ''}`)
      } else {
        toast.error('No readable text found at this link')
      }
    } catch (e: any) {
      const msg = e?.response?.data?.error || e.message || 'JD fetch failed'
  ...
  // #10 — Tailor Apply-per-change (pending staged above; each Apply merges one field).
  const applyTailorField = useCallback((field: 'summary' | 'headline' | 'skills' | 'certifications') => {
    if (!pendingTailor?.[field]) return
    const value = pendingTailor[field]
    setData(prev => {
      if (field === 'summary') return { ...prev, personalInfo: { ...prev.personalInfo, summary: value }, updatedAt: new Date().toISOString() }
      if (field === 'headline') return { ...prev, personalInfo: { ...prev.personalInfo, headline: value }, updatedAt: new Date().toISOString() }
      if (field === 'skills') return { ...prev, skills: Array.isArray(value) ? value.slice(0, 16) : prev.skills, updatedAt: new Date().toISOString() }
      return { ...prev, certifications: value, updatedAt: new Date().toISOString() }
    })
    setPendingTailor((prev: any) => {
      if (!prev) return prev
      const next = { ...prev }
      delete next[field]
      return next
  ...
  const applyTailorBullets = useCallback((expId: string) => {
    const entry = pendingTailor?.experienceBullets?.find((e: any) => e.id === expId)
    if (!entry) return
    setData(prev => ({ ...prev, experience: prev.experience.map(e => e.id === expId ? { ...e, bullets: entry.bullets } : e), updatedAt: new Date().toISOString() }))
    setPendingTailor((prev: any) => {
      if (!prev) return prev
      const rest = (prev.experienceBullets || []).filter((e: any) => e.id !== expId)
      return { ...prev, experienceBullets: rest }
    })
    toast.success('Applied bullets for role')
  }, [pendingTailor])

  const applyAllTailor = useCallback(() => {
    if (!pendingTailor) return
    setData(prev => {
      const next: ResumeData = { ...prev, updatedAt: new Date().toISOString() }
      if (pendingTailor.summary) next.personalInfo = { ...next.personalInfo, summary: pendingTailor.summary }
      if (pendingTailor.headline) next.personalInfo = { ...next.personalInfo, headline: pendingTailor.headline }
      if (Array.isArray(pendingTailor.skills)) next.skills = pendingTailor.skills.slice(0, 16)
      if (Array.isArray(pendingTailor.experienceBullets)) {
        next.experience = next.experience.map(e => {
          const found = pendingTailor.experienceBullets.find((x: any) => x.id === e.id)
          return found ? { ...e, bullets: found.bullets } : e
        })
      }
      if (Array.isArray(pendingTailor.certifications)) (next as any).certifications = pendingTailor.certifications
      if (pendingTailor.rawText) next.personalInfo = { ...next.personalInfo, summary: String(pendingTailor.rawText).slice(0, 400) }
      return next
    })
    setPendingTailor(null)
    setPreTailor(null)
    toast.success('All tailor changes applied ✨')
  }, [pendingTailor])

  const discardTailor = useCallback(() => {
    setPendingTailor(null)
    setPreTailor(null)
    toast('Tailor preview discarded')
  }, [])

  // #10 — ATS missing-keyword Apply: add one keyword to skills (truthful-only reminder).
  const applyAtsKeyword = useCallback((keyword: string) => {
    const kw = keyword.trim()
    if (!kw) return
    setData(prev => {
      if (prev.skills.map(s => s.toLowerCase()).includes(kw.toLowerCase())) return prev
      return { ...prev, skills: [...prev.skills, kw.charAt(0).toUpperCase() + kw.slice(1)].slice(0, 16), updatedAt: new Date().toISOString() }
    })
    toast.success(`Added “${kw}” to skills — keep only what you know`)
  }, [])

  // #10 — Metric Coach Apply (grounded placeholder rewrite, user fills real numbers).
  const applyMetricFlag = useCallback((flagId: string) => {
    setData(prev => applyMetricSuggestion(prev, flagId))
    toast.success('Quantify suggestion applied — fill the [metric]')
  }, [])

  // #10 — Versions (localStorage snapshots).
  const handleSaveVersion = useCallback(() => {
    const v = saveVersion(userId, data, `v${versions.length + 1} — ${data.template || 'resume'}`)
    setVersions(listVersions(userId))
    toast.success(`Snapshot saved (${v.label})`)
  }, [userId, data, versions.length])

  const handleRestoreVersion = useCallback((id: string) => {
    const restored = restoreVersion(userId, id)
    if (!restored) {
      toast.error('Snapshot not found')
      return
    }
    setData({ ...restored, updatedAt: new Date().toISOString() })
    toast.success('Snapshot restored — save a new snapshot before further edits to keep history')
  }, [userId])

  const handleDeleteVersion = useCallback((id: string) => {
    setVersions(deleteVersion(userId, id))
  ...
  const handleGenerateCover = useCallback(async () => {
    ...
    try {
      const res = await resumeAPI.coverLetter({ data, jobDescription: jdText, tone: coverTone })
      setCoverLetter(res.letter)
      setCoverUsedAI(res.usedAI)
      toast.success(res.usedAI ? 'Cover letter generated ✨' : 'Cover letter drafted (heuristic — add Groq key for AI polish)')
    } catch (e: any) {
      toast.error(e?.response?.data?.error || 'Cover letter failed')
    } finally {
      setCoverLoading(false)
    }
  }, [data, jdText, coverTone])
  ...
  const handleDownloadCover = useCallback(() => {
    ...
    a.href = url
    a.download = `${(data.personalInfo.fullName || 'Cover_Letter').replace(/\s+/g, '_')}_CoverLetter.txt`
    document.body.appendChild(a); a.click(); a.remove()
    setTimeout(() => URL.revokeObjectURL(url), 2000)
  }, [coverLetter, data.personalInfo.fullName])

  // #10 — GitHub import (public repos → projects; reuse backend, no token).
  const handleFetchGhRepos = useCallback(async () => {
    ...
    setGhLoading(true)
    try {
      const res = await resumeAPI.githubRepos(u, 12)
      setGhRepos(res.repos)
      setGhSelected(new Set(res.repos.slice(0, 3).map(r => r.name)))
      if (!res.repos.length) toast('No public repos found for this user')
      else toast.success(`Found ${res.repos.length} repos — pick to import`)
    } catch (e: any) {
      const msg = e?.response?.data?.error || e.message || 'GitHub fetch failed'
      toast.error(String(msg).slice(0, 160))
      setGhRepos([])
    } finally {
      setGhLoading(false)
  ...
  const handleImportGhRepos = useCallback(() => {
    const picked = ghRepos.filter(r => ghSelected.has(r.name))
    if (!picked.length) {
      toast.error('Select at least one repo')
      return
    }
    const incoming = reposToProjects(picked)
    setData(prev => ({ ...prev, projects: mergeProjects(prev.projects, incoming), updatedAt: new Date().toISOString() }))
    toast.success(`Imported ${incoming.length} project${incoming.length === 1 ? '' : 's'} (duplicates skipped)`)
  ...
  return (
    <div className="max-w-[1280px] mx-auto">
      ...
      <div className="mb-4 bg-gradient-to-br from-violet-600 via-indigo-600 to-primary-600 rounded-2xl p-[1px] shadow-e2">
        <div className="bg-white dark:bg-night-800 rounded-[15px] p-3 sm:p-4">
          ...
          <div className="mt-3 flex flex-wrap gap-2 items-center">
            ...
            <button onClick={()=>{ setShowCoverModal(true)}} className="inline-flex items-center gap-1.5 px-3.5 py-2 rounded-full bg-rose-50 dark:bg-rose-500/10 border border-rose-200 dark:border-rose-500/20 text-xs font-semibold text-rose-700 dark:text-rose-300 hover:bg-rose-100 dark:hover:bg-rose-500/15">
              <Mail size={12}/> Cover Letter
            </button>
            <button onClick={()=>{ setShowGhModal(true)}} className="inline-flex items-center gap-1.5 px-3.5 py-2 rounded-full bg-surface-900 dark:bg-night-50 text-white dark:text-night-900 text-xs font-semibold hover:opacity-90">
              <Github size={12}/> Import GitHub
            </button>
          </div>
        </div>
      </div>

      {/* #10 — Tailor word-diff preview with Apply-per-change (staged, not auto-applied) */}
      {pendingTailor && (
        <div className="mb-4 bg-white dark:bg-night-800 rounded-2xl border border-violet-200 dark:border-violet-500/25 p-4 shadow-sm">
          <div className="flex flex-wrap items-center justify-between gap-2 mb-3">
            <p className="text-sm font-bold text-surface-900 dark:text-night-50 flex items-center gap-2">
              <Sparkles size={15} className="text-violet-600" /> Tailor preview — review each change, apply what’s truthful
            </p>
            <div className="flex gap-2">
              <button onClick={applyAllTailor} className="inline-flex items-center gap-1.5 px-3.5 py-2 rounded-full bg-gradient-to-r from-violet-600 to-indigo-600 text-white text-xs font-semibold hover:from-violet-700 hover:to-indigo-700">
                <Check size={12} /> Apply all
              </button>
              <button onClick={discardTailor} className="inline-flex items-center gap-1.5 px-3.5 py-2 rounded-full border border-surface-200 dark:border-night-600 text-xs font-medium text-surface-600 dark:text-night-300 hover:bg-surface-50 dark:hover:bg-night-700">
                <X size={12} /> Discard
              </button>
            </div>
          </div>
          {pendingTailor.rawText ? (
            <p className="text-xs text-surface-600 dark:text-night-300 bg-surface-50 dark:bg-night-700 rounded-xl px-3 py-2 whitespace-pre-wrap">{pendingTailor.rawText}</p>
          ) : (
            <div className="space-y-2">
              {pendingTailor.summary && preTailor && (
                <TailorChangeRow label="Summary" before={preTailor.personalInfo.summary || ''} after={pendingTailor.summary} onApply={() => applyTailorField('summary')} />
              )}
              {pendingTailor.headline && preTailor && (
                <TailorChangeRow label="Headline" before={preTailor.personalInfo.headline || ''} after={pendingTailor.headline} onApply={() => applyTailorField('headline')} />
              )}
              {Array.isArray(pendingTailor.skills) && preTailor && (
                <TailorChangeRow label="Skills" before={(preTailor.skills || []).join(', ')} after={pendingTailor.skills.join(', ')} onApply={() => applyTailorField('skills')} />
              )}
              {(pendingTailor.experienceBullets || []).map((e: any) => {
                const orig = preTailor?.experience.find(x => x.id === e.id)
                if (!orig) return null
                return (
                  <TailorChangeRow key={e.id} label={`Bullets — ${orig.role || 'role'}${orig.company ? ` @ ${orig.company}` : ''}`}
                    before={(orig.bullets || []).join(' ')} after={(e.bullets || []).join(' ')} onApply={() => applyTailorBullets(e.id)} />
                )
              })}
            </div>
          )}
          {!!pendingTailor.suggestions?.length && (
            <p className="mt-2 text-xs text-surface-500 dark:text-night-300 flex items-start gap-1.5"><Lightbulb size={12} className="mt-0.5 shrink-0" />{pendingTailor.suggestions[0]}</p>
          )}
        </div>
      )}

      {/* #10 — Metric Coach: flags bullets lacking numbers, grounded quantify rewrites */}
      <div className="mb-4 bg-white dark:bg-night-800 rounded-2xl border border-surface-200 dark:border-night-600 p-4">
        <div className="flex items-center justify-between gap-2 mb-1">
          <p className="text-sm font-bold text-surface-900 dark:text-night-50 flex items-center gap-2">
            <Target size={15} className="text-amber-600" /> Metric Coach
            <span className={`px-2 py-0.5 rounded-full text-[11px] font-bold ${metricFlags.length ? 'bg-amber-100 dark:bg-amber-500/15 text-amber-700 dark:text-amber-300' : 'bg-emerald-100 dark:bg-emerald-500/15 text-emerald-700 dark:text-emerald-300'}`}>
              {metricFlags.length ? `${metricFlags.length} to quantify` : 'All quantified ✓'}
            </span>
          </p>
        </div>
        <p className="text-xs text-surface-500 dark:text-night-300 mb-2">Bullets with numbers get skimmed first. Suggestions use [bracketed placeholders] — fill your real numbers, never ship fakes.</p>
        {metricFlags.length > 0 && (
          <ul className="space-y-2 max-h-64 overflow-auto pr-1">
            {metricFlags.slice(0, 10).map(f => (
              <li key={f.id} className="rounded-xl border border-surface-200 dark:border-night-600 px-3 py-2">
                <p className="text-[11px] font-bold tracking-wide uppercase text-surface-400 dark:text-night-400">{f.parentTitle}</p>
                <p className="text-xs text-surface-700 dark:text-night-200 mt-0.5 line-clamp-2">“{f.text || '(empty)'}”</p>
                <p className="text-xs text-amber-700 dark:text-amber-300 mt-1">{f.reason}</p>
                <p className="text-xs text-surface-600 dark:text-night-300 mt-1 bg-surface-50 dark:bg-night-700 rounded-lg px-2 py-1.5">→ {f.suggestion}</p>
                <button onClick={() => applyMetricFlag(f.id)} className="mt-1.5 inline-flex items-center gap-1 px-3 py-1.5 rounded-full bg-amber-500 hover:bg-amber-600 text-white text-[11px] font-semibold">
                  <Check size={11} /> Apply suggestion
                </button>
              </li>
            ))}
          </ul>
      ...
      {/* #10 — Version history (localStorage snapshots — no backend model by design) */}
      <div className="mb-4 bg-white dark:bg-night-800 rounded-2xl border border-surface-200 dark:border-night-600 p-4">
        <div className="flex flex-wrap items-center justify-between gap-2">
          <p className="text-sm font-bold text-surface-900 dark:text-night-50 flex items-center gap-2">
            <History size={15} className="text-indigo-600" /> Version history
            <span className="px-2 py-0.5 rounded-full bg-surface-100 dark:bg-night-700 text-[11px] font-bold text-surface-500 dark:text-night-300">{versions.length}/20</span>
          </p>
          <button onClick={handleSaveVersion} className="inline-flex items-center gap-1.5 px-3.5 py-2 rounded-full bg-indigo-600 hover:bg-indigo-700 text-white text-xs font-semibold">
            <Save size={12} /> Save snapshot
          </button>
        </div>
        <p className="text-xs text-surface-500 dark:text-night-300 mt-1">Snapshots stay in this browser (offline-first, same store as Save). Restore any point before risky tailoring.</p>
        {versions.length > 0 && (
          <ul className="mt-2 space-y-1.5 max-h-48 overflow-auto pr-1">
            {versions.map(v => (
              <li key={v.id} className="flex items-center gap-2 rounded-xl border border-surface-200 dark:border-night-600 px-3 py-2">
                <span className="flex-1 min-w-0">
                  <span className="block text-xs font-semibold text-surface-800 dark:text-night-100 truncate">{v.label}</span>
                  <span className="block text-[11px] text-surface-400 dark:text-night-400">{new Date(v.createdAt).toLocaleString()}</span>
                </span>
                <button onClick={() => handleRestoreVersion(v.id)} className="inline-flex items-center gap-1 px-2.5 py-1.5 rounded-lg bg-emerald-50 dark:bg-emerald-500/10 border border-emerald-200 dark:border-emerald-500/20 text-[11px] font-semibold text-emerald-700 dark:text-emerald-300 hover:bg-emerald-100">
                  <RotateCcw size={11} /> Restore
                </button>
                <button onClick={() => handleDeleteVersion(v.id)} className="p-1.5 rounded-lg text-surface-400 hover:text-danger-600 hover:bg-danger-50" aria-label={`Delete ${v.label}`}>
                  <Trash2 size={13} />
                </button>
              </li>
            ))}
        ...
        <div className="fixed inset-0 z-[10000] flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm isolate" onClick={()=>setShowJDModal(false)}>
          <div className="bg-white dark:bg-night-800 rounded-2xl shadow-2xl w-full max-w-2xl max-h-[90vh] flex flex-col overflow-hidden" onClick={e=>e.stopPropagation()}>
            ...
            <div className="p-5 space-y-3 overflow-auto">
              ...
              {/* #10 — JD scrape-from-URL: paste a posting link → verbatim text (grounded, no AI) */}
              <div className="flex gap-2">
                <div className="relative flex-1">
                  <Link2 size={14} className="absolute left-3 top-1/2 -translate-y-1/2 text-surface-400" />
                  <input value={jdUrl} onChange={e=>{ setJdUrl(e.target.value); setJdSource(null) }} placeholder="…or paste a job posting link (https://…)"
                    className="w-full pl-9 pr-3 py-2.5 rounded-xl border border-surface-200 dark:border-night-600 bg-white dark:bg-night-850 text-sm text-surface-900 dark:text-night-50 placeholder-surface-400 focus:outline-none focus:ring-2 focus:ring-primary-500/20" />
                </div>
                <button onClick={handleScrapeJd} disabled={jdScraping || !jdUrl.trim()} className="inline-flex items-center gap-1.5 px-4 py-2.5 rounded-xl bg-surface-900 dark:bg-night-50 text-white dark:text-night-900 text-xs font-semibold hover:opacity-90 disabled:opacity-50 shrink-0">
                  {jdScraping ? <Loader2 size={13} className="animate-spin"/> : <Globe size={13}/>} {jdScraping ? 'Fetching…' : 'Fetch'}
                </button>
              </div>
              {jdSource && <p className="text-[11px] text-emerald-600 dark:text-emerald-400">Fetched verbatim from {jdSource} — {jdText.length} chars. Review before tailoring.</p>}
          ...
          <div className="bg-white dark:bg-night-800 rounded-2xl shadow-2xl w-full max-w-xl max-h-[90vh] flex flex-col overflow-hidden" onClick={e=>e.stopPropagation()}>
            ...
            <div className="p-5 space-y-4 overflow-auto">
              ...
              {!!atsResult.aiMissingKeywords?.length && (
                <div>
                  <p className="text-xs font-bold tracking-widest uppercase text-surface-400 dark:text-night-400 mb-2">Missing keywords (AI) — tap to add truthful ones</p>
                  <div className="flex flex-wrap gap-1.5">
                    {atsResult.aiMissingKeywords.map((k:string)=>(
                      <button key={k} onClick={()=>applyAtsKeyword(k)} title={`Add “${k}” to skills`}
                        className="px-2.5 py-1 rounded-full bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800 text-xs font-medium text-red-700 dark:text-red-300 hover:bg-red-100 dark:hover:bg-red-900/30">+ {k}</button>
                    ))}
                  </div>
                </div>
              )}
        ...
        <div className="fixed inset-0 z-[10000] flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm isolate" onClick={()=>setShowCoverModal(false)}>
          <div className="bg-white dark:bg-night-800 rounded-2xl shadow-2xl w-full max-w-2xl max-h-[90vh] flex flex-col overflow-hidden" onClick={e=>e.stopPropagation()}>
            <div className="px-5 py-4 border-b border-surface-200 dark:border-night-700 flex items-center justify-between">
              ...
              <button onClick={()=>setShowCoverModal(false)} className="p-2 rounded-xl hover:bg-surface-100 dark:hover:bg-night-700"><X size={18}/></button>
            </div>
            <div className="p-5 space-y-3 overflow-auto">
              <p className="text-xs text-surface-500 dark:text-night-300">Generated from your resume + the JD above — grounded, no invented facts. Uses your Groq key when set, heuristic otherwise.</p>
              <div className="flex flex-wrap items-center gap-2">
                {(['professional','enthusiastic','concise'] as const).map(t => (
                  <button key={t} onClick={()=>setCoverTone(t)} className={`px-3 py-1.5 rounded-full text-xs font-semibold border ${coverTone===t ? 'bg-rose-600 text-white border-rose-600' : 'border-surface-200 dark:border-night-600 text-surface-600 dark:text-night-300'}`}>{t}</button>
                ))}
                <button onClick={handleGenerateCover} disabled={coverLoading} className="inline-flex items-center gap-1.5 px-4 py-2 rounded-xl bg-rose-600 hover:bg-rose-700 text-white text-xs font-semibold disabled:opacity-50 ml-auto">
                  {coverLoading ? <Loader2 size={13} className="animate-spin"/> : <Sparkles size={13}/>} {coverLoading ? 'Writing…' : 'Generate'}
                </button>
              </div>
              {coverLetter && (
                <>
                  <textarea value={coverLetter} onChange={e=>setCoverLetter(e.target.value)} rows={14}
                    className="w-full px-3 py-3 rounded-xl border border-surface-200 dark:border-night-600 bg-white dark:bg-night-850 text-sm text-surface-900 dark:text-night-50 focus:outline-none focus:ring-2 focus:ring-rose-500/20 resize-none whitespace-pre-wrap" />
                  <p className="text-[11px] text-surface-400 dark:text-night-400">{coverUsedAI ? 'Written with AI from your resume + JD.' : 'Heuristic draft — add your Groq key in Settings → AI for AI polish.'}</p>
                </>
              )}
            </div>
            <div className="px-5 py-4 border-t border-surface-200 dark:border-night-700 flex justify-end gap-2">
              <button onClick={()=>setShowCoverModal(false)} className="px-4 py-2 rounded-xl border border-surface-200 dark:border-night-600 text-sm font-medium">Close</button>
              {!!coverLetter.trim() && (
                <>
                  <button onClick={async ()=>{ try { await navigator.clipboard.writeText(coverLetter); toast.success('Copied') } catch { toast.error('Copy failed') } }} className="inline-flex items-center gap-1.5 px-4 py-2 rounded-xl border border-surface-200 dark:border-night-600 text-sm font-medium">
                    <Copy size={14}/> Copy
                  </button>
                  <button onClick={handleDownloadCover} className="inline-flex items-center gap-1.5 px-4 py-2 rounded-xl bg-rose-600 hover:bg-rose-700 text-white text-sm font-semibold">
        ...
        <div className="fixed inset-0 z-[10000] flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm isolate" onClick={()=>setShowGhModal(false)}>
          <div className="bg-white dark:bg-night-800 rounded-2xl shadow-2xl w-full max-w-xl max-h-[90vh] flex flex-col overflow-hidden" onClick={e=>e.stopPropagation()}>
            <div className="px-5 py-4 border-b border-surface-200 dark:border-night-700 flex items-center justify-between">
              ...
              <button onClick={()=>setShowGhModal(false)} className="p-2 rounded-xl hover:bg-surface-100 dark:hover:bg-night-700"><X size={18}/></button>
            </div>
            <div className="p-5 space-y-3 overflow-auto">
              <p className="text-xs text-surface-500 dark:text-night-300">Public repos only — imported into Projects (title, description, language + topics, link). Duplicates by title are skipped.</p>
              <div className="flex gap-2">
                <input value={ghUsername} onChange={e=>setGhUsername(e.target.value)} onKeyDown={e=>{ if (e.key==='Enter') handleFetchGhRepos() }} placeholder="GitHub username (e.g., torvalds)"
                  className="flex-1 px-3 py-2.5 rounded-xl border border-surface-200 dark:border-night-600 bg-white dark:bg-night-850 text-sm text-surface-900 dark:text-night-50 placeholder-surface-400 focus:outline-none focus:ring-2 focus:ring-primary-500/20" />
                <button onClick={handleFetchGhRepos} disabled={ghLoading || !ghUsername.trim()} className="inline-flex items-center gap-1.5 px-4 py-2.5 rounded-xl bg-surface-900 dark:bg-night-50 text-white dark:text-night-900 text-xs font-semibold hover:opacity-90 disabled:opacity-50 shrink-0">
                  {ghLoading ? <Loader2 size={13} className="animate-spin"/> : <Github size={13}/>} {ghLoading ? 'Fetching…' : 'Fetch'}
                </button>
              </div>
              {ghRepos.length > 0 && (
                <ul className="space-y-1.5">
                  {ghRepos.map(r => (
                    <li key={r.name}>
                      <label className="flex items-start gap-2.5 rounded-xl border border-surface-200 dark:border-night-600 px-3 py-2 cursor-pointer hover:bg-surface-50 dark:hover:bg-night-700">
                        <input type="checkbox" checked={ghSelected.has(r.name)} onChange={()=>setGhSelected(prev=>{ const next = new Set(prev); if (next.has(r.name)) next.delete(r.name); else next.add(r.name); return next })} className="mt-1 accent-indigo-600" />
                        <span className="flex-1 min-w-0">
                          <span className="block text-xs font-bold text-surface-900 dark:text-night-50">{r.name} {r.stars > 0 && <span className="font-medium text-amber-600">★ {r.stars}</span>}</span>
                          <span className="block text-[11px] text-surface-500 dark:text-night-300 truncate">{r.description || 'No description'}{r.language ? ` • ${r.language}` : ''}</span>
                        </span>
                      </label>
                    </li>
                  ))}
            ...
            <div className="px-5 py-4 border-t border-surface-200 dark:border-night-700 flex justify-end gap-2">
              <button onClick={()=>setShowGhModal(false)} className="px-4 py-2 rounded-xl border border-surface-200 dark:border-night-600 text-sm font-medium">Cancel</button>
              <button onClick={handleImportGhRepos} disabled={!ghSelected.size} className="inline-flex items-center gap-1.5 px-5 py-2 rounded-xl bg-primary-600 hover:bg-primary-700 text-white text-sm font-semibold disabled:opacity-50">
                <Check size={14}/> Import {ghSelected.size ? `(${ghSelected.size})` : ''}
              </button>
            </div>
          </div>
        </div>,
