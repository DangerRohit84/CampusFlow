...
function headingParagraph(text: string, opts?: { color?: string; size?: number; align?: typeof AlignmentType[keyof typeof AlignmentType]; spacingAfter?: number; borderBottom?: boolean; accentHex?: string }): Paragraph {
...
function hr(accentHex?: string): Paragraph {
  ...
  return new Paragraph({
    border: {
      bottom: { color: c, space: 1, style: BorderStyle.SINGLE, size: 6 },
    },
...
function contactParagraph(p: ResumeData['personalInfo'], align: typeof AlignmentType[keyof typeof AlignmentType] = AlignmentType.CENTER, accentHex?: string): Paragraph {
  const children: any[] = []
  ...
  parts.forEach((part, idx) => {
    ...
    if (part.url) {
      const link = new ExternalHyperlink({
        children: [new TextRun({ text: part.text, color: accentHex ? hexNoHash(accentHex) : '1D4ED8', size: 16, style: 'Hyperlink' })],
        link: part.url,
      })
      children.push(link)
    } else {
      children.push(new TextRun({ text: part.text, color: '374151', size: 16 }))
    }
  })
...
function summaryParagraph(text: string): Paragraph {
  return new Paragraph({
    spacing: { after: 120 },
    children: [new TextRun({ text, size: 19, color: '1F2937' })],
  })
}

function skillsFlatParagraph(skills: string[]): Paragraph {
  return new Paragraph({
    spacing: { after: 120 },
    children: [new TextRun({ text: skills.join(', '), size: 19, color: '1F2937' })],
  })
}

function skillRowsParagraphs(rows: { label: string; value: string }[], accentHex?: string): Paragraph[] {
  const out: Paragraph[] = []
  for (const r of rows) {
    out.push(
      new Paragraph({
        spacing: { after: 40 },
        children: [
          new TextRun({ text: `${r.label}: `, bold: true, size: 19, color: accentHex ? hexNoHash(accentHex) : '111827' }),
          new TextRun({ text: r.value, size: 19, color: '1F2937' }),
        ],
        indent: { left: 120 },
      })
    )
...
function experienceParagraphs(exps: ResumeData['experience'], opts?: { accentHex?: string; density?: string }): Paragraph[] {
  const out: Paragraph[] = []
  ...
  for (const exp of exps) {
    ...
    out.push(
      new Paragraph({
        ...
        spacing: { after: 20 },
        children: [
          new TextRun({ text: `${role} — ${company}${loc}`, bold: true, size: 20, color: '111827' }),
          new TextRun({ text: '\t' }),
          new TextRun({ text: dateRange, italics: true, size: 17, color: accent }),
        ],
      })
    )
      ...
      if (idx > 1 && idx < 80) {
        ...
        out.push(
          new Paragraph({
            ...
            spacing: { after: 20 },
            children: [
              new TextRun({ text: before + ' ', bold: true, size: 19, color: '1F2937' }),
              new TextRun({ text: after, size: 19, color: '1F2937' }),
            ],
          })
        )
      } else {
        out.push(
          new Paragraph({
            bullet: { level: 0 },
            spacing: { after: 20 },
            children: [new TextRun({ text: b, size: 19, color: '1F2937' })],
          })
        )
      }
    }
...
function projectsParagraphs(projects: ResumeData['projects'], opts?: { accentHex?: string }): Paragraph[] {
  const out: Paragraph[] = []
  ...
  for (const proj of projects) {
    ...
    out.push(
      new Paragraph({
        ...
        spacing: { after: 20 },
        children: [
          new TextRun({ text: title, bold: true, size: 20, color: '111827' }),
          new TextRun({ text: '\t' }),
          new TextRun({ text: dateStr, italics: true, size: 17, color: '6B7280' }),
        ],
      })
    )
    if (tech) {
      out.push(new Paragraph({ spacing: { after: 40 }, children: [new TextRun({ text: tech, italics: true, size: 17, color: accent })] }))
    }
    if (proj.description?.trim()) {
      out.push(new Paragraph({ spacing: { after: 40 }, children: [new TextRun({ text: proj.description.trim(), size: 19, color: '1F2937' })] }))
    }
    if (proj.link?.trim()) {
      ...
      out.push(
        new Paragraph({
          spacing: { after: 60 },
          children: [new ExternalHyperlink({ children: [new TextRun({ text: url, color: accent, size: 17, style: 'Hyperlink' })], link: url })],
        })
      )
...
function educationParagraphs(eds: ResumeData['education'], opts?: { accentHex?: string }): Paragraph[] {
  const out: Paragraph[] = []
  ...
  for (const ed of eds) {
    ...
    out.push(
      new Paragraph({
        ...
        spacing: { after: 20 },
        children: [
          new TextRun({ text: `${degree} — ${school}${loc}`, bold: true, size: 20, color: '111827' }),
          new TextRun({ text: '\t' }),
          new TextRun({ text: dateRange, italics: true, size: 17, color: accent }),
        ],
      })
...
function certificationsParagraphs(certs: NonNullable<ResumeData['certifications']>, opts?: { accentHex?: string }): Paragraph[] {
  const out: Paragraph[] = []
  const accent = opts?.accentHex ? hexNoHash(opts.accentHex) : '374151'
  for (const c of certs) {
    ...
    out.push(
      new Paragraph({
        ...
        spacing: { after: 20 },
        children: [
          new TextRun({ text: `${name}${issuer}`, bold: true, size: 20, color: '111827' }),
          new TextRun({ text: '\t' }),
          new TextRun({ text: date, size: 17, color: accent }),
        ],
      })
    )
    if (c.url?.trim()) {
      const url = c.url.trim()
      out.push(new Paragraph({ spacing: { after: 60 }, children: [new ExternalHyperlink({ children: [new TextRun({ text: url, color: accent, size: 17 })], link: url })] }))
    } else out.push(spacer(40))
  }
  return out
}

function buildParagraphsForTemplate(data: ResumeData, template: ResumeTemplateId): Paragraph[] {
  ...
  if (template === 'source-split') {
    ...
    paras.push(
      new Paragraph({
        spacing: { after: 40 },
        children: [new TextRun({ text: p.fullName || 'Jane Doe', bold: true, size: 50, color: '000000' })], // 24.8pt ~ 50 half-pt
      })
    )
    if (p.location?.trim()) {
      paras.push(new Paragraph({ alignment: AlignmentType.RIGHT, spacing: { after: 40 }, children: [new TextRun({ text: p.location.trim(), size: 19, color: '000000' })] }))
    }
    ...
    paras.push(new Paragraph({ spacing: { after: 120 }, children: [new TextRun({ text: headline, bold: true, size: 20, color: '000000', allCaps: true })] }))
    ...
    paras.push(new Paragraph({ alignment: AlignmentType.CENTER, spacing: { after: 80 }, children: [new TextRun({ text: hl, bold: true, size: 17, color: '000000', allCaps: true })] }))
    ...
    paras.push(new Paragraph({ alignment: AlignmentType.CENTER, spacing: { after: 80 }, children: [new TextRun({ text: hlText, bold: true, size: 22, color: hexNoHash(accentHex), allCaps: headingStyle === 'uppercase' })] }))
    paras.push(contactParagraph(p, AlignmentType.CENTER, accentHex))
  ...
  const sectionBuilders: Record<CustomSectionId, () => Paragraph[]> = {
    summary: () => {
  ...
  paras.push(
    new Paragraph({
      ...
      spacing: { before: 240 },
      children: [new TextRun({ text: 'Generated by CampusFlow', size: 14, color: '9CA3AF', italics: true })],
    })
  )
  return paras
}
          ...
          levels: [
            {
              ...
              style: {
                paragraph: { indent: { left: convertInchesToTwip(0.2), hanging: convertInchesToTwip(0.15) } },
              },
            },
          ],
        },
