 ...
 * this locks the contract (limit 1..50, page >=1, envelope shape) so the
 * AdminPage Users pager UI can rely on {data, pagination:{page,limit,total,pages,nextCursor}}.
 * Mirrors admin.ts inline parsing exactly — do not diverge without updating callers.
 */

/** Users tab page size — matches backend take:50 cap (10k scale). */
...
export function parseListPagination(query: {
  ...
  const limit = Number.isFinite(limitParam)
    ? Math.min(PAGE_LIMIT_MAX, Math.max(1, limitParam))
    : DEFAULT_PAGE_LIMIT
