/**
 * CampusFlow Resume Studio — LaTeX vector generator (backend)
 * Five Overleaf-mimic templates: source-split, compact, classic (Jake), modern (AltaCV two-col), minimal (ATS clean)
 * Keep parity with frontend apps/web/src/lib/resumeLatex.ts
 */
...
export type ResumeDataForLatex = {
...
function renderContactLine(p: ResumePersonalInfo): string {
  const parts: string[] = []
  if (p.email?.trim()) {
...
function renderSkillsInline(skills: string[]): string {
  ...
  return skills.map(s => escapeLatex(String(s).trim())).filter(Boolean).join(', ')
}
...
function generateSourceSplitLatex(data: ResumeDataForLatex): string {
  const p = data.personalInfo || ({} as ResumePersonalInfo)
...
function generateCompactLatex(data: ResumeDataForLatex): string {
  const p = data.personalInfo || ({} as ResumePersonalInfo)
...
function generateClassicLatex(data: ResumeDataForLatex): string {
  const p = data.personalInfo || ({} as ResumePersonalInfo)
  ...
  const hasExperience = Boolean(data.experience && data.experience.length > 0)
  const hasEducation = Boolean(data.education && data.education.length > 0)
  ...
  const lines: string[] = []
  lines.push(`% !TEX program = pdflatex`)
  lines.push(`% Generated by CampusFlow — Jake's Resume (Classic)`)
  ...
  if (hasProjects) {
    ...
    for (const proj of data.projects) {
      const title = escapeLatex((proj.title || 'Untitled Project').trim() || 'Untitled Project')
      const tech = proj.tech && proj.tech.length > 0 ? `\\textit{${proj.tech.map(t => escapeLatex(t)).join(' $\\cdot$ ')}}` : ''
      const dateStr = proj.date?.trim() ? `\\hfill ${escapeLatex(proj.date.trim())}` : ''
      const desc = proj.description?.trim() ? escapeLatex(proj.description.trim()) : ''
      const link = proj.link?.trim() ? `\\href{${fmtUrlForHref(proj.link.trim())}}{${escapeLatex(proj.link.trim())}}` : ''
      if (dateStr || tech) {
...
function generateModernLatex(data: ResumeDataForLatex): string {
  const p = data.personalInfo || ({} as ResumePersonalInfo)
  const name = escapeLatex(p.fullName?.trim() || 'Your Name')
  const headline = p.headline?.trim() ? escapeLatex(p.headline.trim()) : ''
  const summary = p.summary?.trim() ? formatSummary(p.summary) : ''
  ...
  const lines: string[] = []
  lines.push(`% !TEX program = pdflatex`)
  lines.push(`% Generated by CampusFlow — AltaCV Modern (two-column)`)
  lines.push(`\\documentclass[11pt,a4paper]{article}`)
  ...
  const linkIcons = (p.links || []).filter(l=>l.url?.trim()).map(l=>{
    const label = escapeLatex(l.label||'Link')
    const url = fmtUrlForHref(l.url.trim())
    const icon = /github/i.test(label) ? '\\faGithub' : /linkedin/i.test(label) ? '\\faLinkedin' : '\\faLink'
...
function generateMinimalLatex(data: ResumeDataForLatex): string {
  const p = data.personalInfo || ({} as ResumePersonalInfo)
  const name = escapeLatex(p.fullName?.trim() || 'Your Name')
  const headline = p.headline?.trim() ? escapeLatex(p.headline.trim()) : ''
  const contactLine = renderContactLine(p)
  ...
  const lines: string[] = []
  lines.push(`% !TEX program = pdflatex`)
  lines.push(`% Generated by CampusFlow — ATS Minimal (Clean)`)
  lines.push(`\\documentclass[11pt,a4paper]{article}`)
  lines.push(`\\usepackage[utf8]{inputenc}`)
  lines.push(`\\usepackage[T1]{fontenc}`)
...
function generateCustomLatex(data: ResumeDataForLatex & { customConfig?: any }): string {
  ...
  const lines: string[] = []
  lines.push(`% !TEX program = pdflatex`)
  lines.push(`% Generated by CampusFlow — Custom (Fully Editable) — accent ${accentHex}`)
  lines.push(`\\documentclass[10pt,a4paper]{article}`)
  lines.push(`\\usepackage[utf8]{inputenc}`)
  lines.push(`\\usepackage[T1]{fontenc}`)
  lines.push(fontPkg)
  ...
  lines.push(`\\setlength{\\parindent}{0pt}`)
  lines.push(`\\setlength{\\parskip}{0pt}`)
  if (showDividers) lines.push(`\\titleformat{\\section}{\\vspace{-2pt}\\color{accent}\\raggedright\\large\\bfseries}{}{0em}{}[\\vspace{-2pt}{\\color{accent}\\hrule height 0.6pt}\\vspace{-5pt}]`)
  else lines.push(`\\titleformat{\\section}{\\vspace{-2pt}\\color{accent}\\raggedright\\large\\bfseries}{}{0em}{}`)
  ...
  for (const id of order) {
    if (id === 'summary' && should('summary') && p.summary?.trim()) { lines.push(`\\section{Summary}`); lines.push(formatSummary(p.summary)); lines.push(``) }
    else if (id === 'skills' && should('skills') && (data.skills||[]).length) {
      const rows = parseSkillRowsForLatex(data.skills||[])
      lines.push(`\\section{Technical Skills}`)
      if (rows.length===1 && rows[0].label==='Skills') lines.push(rows[0].value)
      else { lines.push(`\\begin{tabular*}{\\textwidth}{@{ } p{0.22\\textwidth} @{\\hspace{6pt}} p{0.76\\textwidth} @{}}`); for (const r of rows) lines.push(`  \\textbf{${r.label}:} & ${r.value} \\\\`); lines.push(`\\end{tabular*}`) }
      lines.push(``)
    }
      ...
      for (const exp of data.experience) {
        ...
        if (bullets.length) { lines.push(`\\begin{itemize}[nosep]`); for (const b of bullets) lines.push(`  \\item ${formatBulletLatex(b)}`); lines.push(`\\end{itemize}`) } else lines.push(`\\vspace{2pt}`)
        lines.push(`\\vspace{4pt}`)
      }
      lines.push(``)
    }
...
export function generateResumeLatex(data: ResumeDataForLatex): string {
  const tpl = String((data as any).template || 'source-split').toLowerCase() as 'source-split'|'compact'|'classic'|'modern'|'minimal'|'custom'
  if (tpl === 'custom') return generateCustomLatex(data as any)
...
export default generateResumeLatex
